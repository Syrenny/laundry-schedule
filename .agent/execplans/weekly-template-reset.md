...
- `SCHEDULE_RESET_TRIGGER_DAY`: день trigger, по умолчанию `MONDAY`.
...
При запуске `installWeeklyResetTrigger` старые trigger для `resetWeeklySchedule` должны удалиться, затем должен появиться один еженедельный trigger на день и час из настроек. Повторный запуск не должен создавать дубликаты.
